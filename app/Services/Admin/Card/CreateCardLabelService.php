    ...
    protected function getFirstLine(): string
    {
        ...
        if (
            ...
            if ($this->cardProduct->language === 'English') {
                ...
                } elseif ($this->year < 2002 or $this->cardSetName == 'NEO DESTINY') {
            ...
            if ($this->cardProduct->language === 'Japanese') {
                ...
                } elseif (Carbon::parse($this->year)->year <= 2001 and $this->cardSeries->name === 'E-CARD ERA') {
                    $label_line_one = [$this->year, $this->category->name, $language];
                } elseif (Carbon::parse($this->year)->year >= 2001 and $this->cardSeries->name !== 'NEO ERA') {
            ...
            if ($this->cardSeriesName === 'HOLIDAY SERIES' || Str::contains($this->cardSetName, ['DECK', 'BOX TOPPER', 'PIN CLUB'])) {
                ...
                } elseif (Str::contains($this->cardSetName, 'DECK') and $full_date->day === 30 and $full_date->month === 7 and $full_date->year === 2021) {
                    $label_line_one = [$this->year, $this->category->name, 'CN'];
                } elseif (Str::contains($this->cardSetName, 'DECK') and $full_date->day === 22 and $full_date->month === 10 and $full_date->year === 2021) {
                    $label_line_one = [$this->year, $this->category->name, 'NF'];
                } elseif (Str::contains($this->cardSetName, 'PIN CLUB') and $full_date->day === 17 and $full_date->month === 9 and $full_date->year === 2021) {
                    $label_line_one = [$this->year, $this->category->name, 'CN'];
                } else {
                    $label_line_one = [$this->year, $this->category->name];
                }
            } elseif ($this->cardSeriesName === 'CRYPTID NATION' and ! Str::contains($this->cardSetName, 'PROMOS')) {
