...
class SendShipmentTrackingReminderEmail implements ShouldQueue
{
    ...
    public function handle(OrderPaid $event)
    {
        ...
        $this->emailService->scheduleEmail((now()->addMinutes(2)), $event->order->user->email, $event->order->user->getFullName(), $this->emailService->getSubjectByTemplate($template), $template, ['FIRST_NAME' => $event->order->user->first_name]);
    }
}
