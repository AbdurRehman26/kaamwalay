...
it('validates reports data for weekly, monthly and quarterly', function ($reportable) {
    ...
    $resultArray = [
        ...
        'Average number of days taken from confirmation to grading' => (int) $queryCardsConfirmationToGrading->select(DB::raw("AVG(orders.graded_at) - AVG(order_status_histories.created_at) as avg"))
        ->where('orders.order_status_id', '>=', OrderStatus::GRADED)
        ->whereBetween("orders.graded_at", [$fromDate, $toDate])
        ->first()->avg . ' Day(s)',

        'Average number of days taken from confirmation to shipping' => (int) $queryCardsConfirmationToGrading->select(DB::raw("AVG(orders.shipped_at) - AVG(order_status_histories.created_at) as avg"))
            ->where('orders.order_status_id', '>=', OrderStatus::SHIPPED)
            ->whereBetween("orders.shipped_at", [$fromDate, $toDate])
            ->first()->avg . ' Day(s)',

        'Average number of days taken from grading to shipping' => (int) Order::select(DB::raw("AVG(shipped_at) - AVG(graded_at) as avg"))->betweenDates($fromDate, $toDate)->first()->avg . ' Day(s)',
        'Average time from submission to payment' => (int) Order::select(DB::raw("AVG(paid_at) - AVG(created_at) as avg"))->betweenDates($fromDate, $toDate)->first()->avg . ' Day(s)',
    ];
