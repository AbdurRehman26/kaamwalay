...
export interface SalesRepNewOrderSliceState {
...
const initialState: SalesRepNewOrderSliceState = {
    userId: -1,
    user: {
        fullName: '',
        email: '',
...
export const getCountriesList = createAsyncThunk('salesRepCreateOrderSlice/getCountriesList', async () => {
...
export const createOrder = createAsyncThunk('salesRepCreateOrderSlice/createOrder', async (_, { getState }: any) => {
    const currentSubmission: SalesRepNewOrderSliceState = getState().salesRepCreateOrderSlice;
...
export const getSavedAddresses = createAsyncThunk(
    'salesRepCreateOrderSlice/getSavedAddresses',
    async (customerId: number, { getState }: any) => {
        const availableStatesList: any = getState().salesRepCreateOrderSlice.step03Data.availableStatesList;
        const apiService = app(APIService);
        const endpoint = apiService.createEndpoint(`salesman/customer/${customerId}/addresses`);
...
export const getShippingFee = createAsyncThunk(
    'salesRepCreateOrderSlice/getShippingFee',
    async (selectedCards: SearchResultItemCardProps[], thunk) => {
        const apiService = app(APIService);
        const endpoint = apiService.createEndpoint('salesman/orders/shipping-fee');
        const shippingMethod = (thunk.getState() as any).salesRepCreateOrderSlice.shippingMethod;
        const shippingAddress = (thunk.getState() as any).salesRepCreateOrderSlice.step03Data.selectedAddress;
        const existingAddresses = (thunk.getState() as any).salesRepCreateOrderSlice.step03Data.selectedExistingAddress;
...
export const getPaymentMethod = createAsyncThunk('salesRepCreateOrderSlice/getPaymentMethod', async () => {
    const apiService = app(APIService);
    const endpoint = apiService.createEndpoint('salesman/orders/payment-methods?submission_create=true');
    const paymentMethod = await endpoint.get('');
    return paymentMethod.data[0];
});

export const getStatesList = createAsyncThunk(
    'salesRepCreateOrderSlice/getStatesList',
...
export const getCustomer = createAsyncThunk('salesRepCreateOrderSlice/getCustomer', async (customerId: number) => {
    const apiService = app(APIService);

    const endpoint = apiService.createEndpoint(`salesman/customers/${customerId}`);
...
export const salesRepCreateOrderSlice = createSlice({
    name: 'salesRepCreateOrderSlice',
...
export const {
    ...
    setShippingFee,
} = salesRepCreateOrderSlice.actions;
