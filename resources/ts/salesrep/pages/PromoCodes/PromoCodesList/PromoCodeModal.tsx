...
import { storeCoupon } from '@shared/redux/slices/salesmanPromoCodesSlice';
...
export function PromoCodeModal() {
    ...
    const modalTitle = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.modalTitle);
    const promoCodeValue = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.promoCode);
    const discountType = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.type);
    const discountValue = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.discountValue);
    const discountApplicationType = useSharedSelector(
        (state) => state.salesmanNewPromoCodeSlice.discountApplicationType,
    );
    const isPermanent = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.isPermanent);
    const usageAllowed = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.usageAllowed);
    const discountStartDate = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.availableFrom);
    const discountEndDate = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.availableTill);
    const showModal = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.showNewPromoCodeDialog);
    const applicables = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.applicables);
    const description = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.description);
    const minThresholdType = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.minThresholdType);
    const minThresholdValue = useSharedSelector((state) => state.salesmanNewPromoCodeSlice.minThresholdValue);
    ...
    const serviceLevelCouponables = useSharedSelector(
        // @ts-ignore
        (state) => state.salesmanNewPromoCodeSlice.applicables[serviceLevelApplicableIndex]?.couponables,
    );
    ...
    const selectedDiscountApplicationServiceLevels = useSharedSelector(
        (state) => state.salesmanNewPromoCodeSlice.selectedDiscountApplicationServiceLevelsIds,
    );
            ...
            return async () => {
                if (applicableCode === 'service_level') {
                    const couponablesEndpoint = apiService.createEndpoint(
                        `salesman/couponable/entities?coupon_applicable_id=${applicableId}`,
                    );
