<section class="Home-section Home-conclusion">
    <div class="container Home-sectionContainer Home-sectionContainer--center-center">
        <div class="Home-sectionColumn Home-sectionColumn--half Home-sectionColumn--center-center">
            <h4 class="Home-sectionContentCaption text-center" data-aos="fade-up" data-aos-easing="ease-in-out">Join the
                Grading Revolution</h4>
            <h3 class="Home-sectionContentHeadline text-center" data-aos="fade-up" data-aos-easing="ease-in-out"
                data-aos-delay="50">Start your submission, today.</h3>
            <a href="/dashboard/submissions/new" class="Home-buttonCta Home-buttonCta--light" data-aos="fade-up" data-aos-easing="ease-in-out"
               data-aos-delay="100">Start a Submission</a>
        </div>
    </div>

    <div class="container Home-sectionContainer">
        <div class="Home-cardImages">
            <div class="Home-cardColumn hide-tablet">
                <img src="{{ asset('assets/images/landings/home/card01.jpg') }}" alt="" data-aos="fade"
                     data-aos-easing="ease-in-out" class="Home-cardImage" />
            </div>
            <div class="Home-cardColumn">
                <img src="{{ asset('assets/images/landings/home/card02.jpg') }}" alt="" data-aos="fade"
                     data-aos-easing="ease-in-out" data-aos-delay="50" class="Home-cardImage" />
            </div>
            <div class="Home-cardColumn">
                <img src="{{ asset('assets/images/landings/home/card03.jpg') }}" alt="" data-aos="fade"
                     data-aos-easing="ease-in-out" class="Home-cardImage" />
            </div>
            <div class="Home-cardColumn hide-tablet">
                <img src="{{ asset('assets/images/landings/home/card04.jpg') }}" alt="" data-aos="fade"
                     data-aos-easing="ease-in-out" data-aos-delay="50" class="Home-cardImage" />
            </div>
        </div>
    </div>
</section>
